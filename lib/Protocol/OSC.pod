...
Creates Protocol::Instance with optional C<actions> argument which is hashref of pairs: C<< path => coderef >> and optional default scheduler for C<process> method - see below.

=head1 METHODS

=head2 message($path, $typetag, ?@args)

Encodes message to packet. Typetag supports these OSC-types: C<ifstdbht>. Everything else (like C<TFNI>) will not affect packing of C<@args>.
...
Parses OSC packet/data and process messages in it. It will call matched actions throught C<$scheduler_cb> which is 
just C<< sub { $_[0]->(splice @_,1) } >> by default(or specifed in constructor). 
Arguments to scheduler are C<$action_coderef, $time, $action_path, $osc_msg, ?@osc_bundles>.

=head2 actions

Returns hashref of actions: C<< path => coderef >> pairs. One could modify this hashref or use methods below.

=head2 set_cb($path, $cb)

Set coderef C<$cb> to C<actions>

=head2 del_cb($path)

Remove coderef at C<$path> from C<actions> at C<$path>
...
Returns list of raw OSC data packets in C<$buf> from stream buffer and residue of $buf.
